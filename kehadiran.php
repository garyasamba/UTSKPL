	...
	if(isset($_POST['employee'])){
		...
		if($query->num_rows > 0){
			...
			if($status == 'in'){
				...
				if($query->num_rows > 0){
					$output['error'] = true;
					$output['message'] = 'Anda telah masuk pada hari ini';
				}
				else{
					//updates
					...
					if($conn->query($sql)){
						$output['message'] = 'Jam Masuk: '.$row['firstname'].' '.$row['lastname'];
					}
					else{
						$output['error'] = true;
				...
				if($query->num_rows < 1){
					...
					if($row['time_out'] != '00:00:00'){
						$output['error'] = true;
						$output['message'] = 'Anda telah keluar pada hari ini';
					}
					else{
						
						$sql = "UPDATE attendance SET time_out = NOW() WHERE id = '".$row['uid']."'";
						if($conn->query($sql)){
							$output['message'] = 'Jam Keluar: '.$row['firstname'].' '.$row['lastname'];
